  ...
  useEffect(() => {
    ...
    const applyTransform = (force = false) => {
      ...
      layersRef.current.forEach((entry) => {
        ...
        const vertical = entry.startOffset + (entry.endOffset - entry.startOffset) * scrollProgress;
        const dynamicScale = Math.max(entry.minScale, entry.scale - entry.deltaScale * scrollProgress);
